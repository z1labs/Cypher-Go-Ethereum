...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, runMode core.MessageRunMode, isGasEstimation, isEthCall bool) (*core.ExecutionResult, error) {
...
func runScheduledTxes(ctx context.Context, b core.NodeInterfaceBackendAPI, state *state.StateDB, header *types.Header, blockCtx vm.BlockContext, runMode core.MessageRunMode, result *core.ExecutionResult) (*core.ExecutionResult, error) {
	...
	for runMode == core.MessageGasEstimationMode && len(scheduled) > 0 {
		...
		evm := b.GetEVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true, IsGasEstimation: false, IsEthCall: true}, &blockCtx)
		go func() {
			<-ctx.Done()
			evm.Cancel()
		}()
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, runMode core.MessageRunMode, isGasEstimation, isEthCall bool) (*core.ExecutionResult, error) {
	...
	return doCall(ctx, b, args, state, header, overrides, blockOverrides, timeout, globalGasCap, runMode, isGasEstimation, isEthCall)
}
...
func (s *BlockChainAPI) Call(ctx context.Context, args TransactionArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides) (hexutil.Bytes, error) {
	...
	result, err := DoCall(ctx, s.b, args, *blockNrOrHash, overrides, blockOverrides, s.b.RPCEVMTimeout(), s.b.RPCGasCap(), core.MessageEthcallMode, false, true)
	if err != nil {
		if client := fallbackClientFor(s.b, err); client != nil {
			var res hexutil.Bytes
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		var vmenv = b.GetEVM(ctx, msg, statedb, header, &config, nil)
